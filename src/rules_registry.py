    ...
    def require_verified(self, rule_paths: Iterable[str]) -> None:
        ...
        for path in rule_paths:
            ...
            if not supporting or not any(
                source.verification_status in acceptable for source in supporting
            ):
                missing.append(path)
        if missing:
            raise RulesValidationError(
...
class RulesRegistry:
        ...
        if season_id in self._cache:
            ...
            if not path.exists():
                raise UnknownSeasonError(
                    f"No NHL ruleset found for {season_id}. Available: "
                    f"{', '.join(self.available_seasons())}"
                )
            try:
    ...
    def _parse_and_validate(payload: Mapping[str, Any], path: Path) -> SeasonRules:
        ...
        if salary["maximum_contract_years_new_club"] > salary[
            "maximum_contract_years_re_signing"
        ]:
            raise RulesValidationError(
                "new-club maximum term cannot exceed re-signing maximum term"
            )
