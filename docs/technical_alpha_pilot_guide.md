...
15. Continue only when the launcher returns `"status": "pass"`, `"installation_confirmed": true`, `"launch_confirmed": true`, `"launch_stability_confirmed": true`, and `"launch_stability_seconds": 3.0`. This proves the installed package launched and the same Android process survived the required three-second stability window on the authorized device without publishing the device serial. Treat any process restart or missing stability evidence as a blocker; do not begin gameplay smoke.
15a. Before beginning gameplay, verify the tester launch handoff card reports backend status `Ready` and that `Start Test` is enabled. If the card reports `Unavailable` or `Maintenance`, stop the session, keep `Start Test` disabled, restore/requalify the endpoint, and repeat the post-launch backend check before continuing. Do not record gameplay smoke against a launch card that was not in the `Ready` state.
16. On that same installed package, complete the full required smoke route in order: New Game, franchise selection, advance day, roster, standings, one trade attempt plus Trade History verification, save, reload, debug report, and reset. Also confirm health and season context before recording the route as passed. Do not substitute a partial API-only check for this physical-device route.
