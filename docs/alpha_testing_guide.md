...
9. **Save/reload checkpoint:** the facilitator restarts the certified API while the tester relaunches the mobile client; the season day, results, team selection, and trade history should persist. The tester must not run backend or repository commands. Record this checkpoint as passed only after all four persisted values are visibly restored after restart.
